  ...
  <div class="popular-products <?php if($hide_section == true) { echo 'display-none'; }?> <?php if($background == 'true') { echo 'popular-products--background'; }?>">
    ...
    <?php if (!empty($title)): ?>
      <span class="popular-products__decoration-title">
        <?php 
          $cleanTitle = preg_replace('/<span[^>]*>.*?<\/span>/si', '', $title);
          echo apply_filters('the_title', $cleanTitle);
        ?>
      </span>
    <?php endif; ?>
    <div class="container">
      ...
      <div class="popular-products__wrapper">
        <div class="row popular-products__row">
          <?php while ($loop->have_posts()): $loop->the_post(); 
            ...
            $product_advance = get_field('zaliczka', get_the_ID());
          ?>
            <li class="popular-products__item product <?php if (in_array('ksiazki', $product_categories)) { echo 'product--ksiazki'; } ?>">
              ...
              <div class="product__button-wrapper">
                ...
                if ( $product->is_on_sale() ) {
                    ...
                    echo '<div class="product__omnibus"><p>Najniższa cena z ostatnich 30 dni: <span>' . wc_price( $sale_price ) . '</span></p></div>';
                }
                ?>
                <?php woocommerce_template_loop_add_to_cart(); ?>

                <?php if (in_array('akademia', $product_categories) && !empty($product_advance)): ?>
                  <a href="<?php echo esc_html($product_advance);?>" class="button product__advance">Wpłać zaliczkę</a>
                <?php endif; ?>
              </div>
